...
def loss_function(recon_x, x, mu, logvar, model, z):
    BCE = reconstruction_function(recon_x, x.reshape(-1,784))
...
def test(epoch, prior):
    ...
    for data, lab in test_loader:
        ...
        recon_batch, mu, logvar, z = model(data)
        test_loss += loss_function(recon_batch, data, mu, logvar, prior, z).item()
    ...
    regen = model.sample(prior, 20).cpu().data.numpy()
    fig, ax = plt.subplots(4, 5)
    for i, ax in enumerate(ax.flatten()):
        plottable_image = np.reshape(regen[i], (28, 28))
